...
function nminvoi(pa::Vector{Int}, pb::Vector{Int})
	...
	2.0*Iab/(Ha+Hb), (Ha + Hb - 2Iab)/(Ha + Hb - Iab)
end

function rnmi(p1::Vector{Int}, p2::Vector{Int}, N::Int=10)
    pa = copy(p1)
    pb = copy(p2)
...
function compute_rnmi(pa::Vector{Int}, pb::Vector{Int})
    length(pa) == length(pb) || error("Two partitions have different number of nodes !")
    n = length(pa)
    qa = maximum(pa)
    qb = maximum(pb)
